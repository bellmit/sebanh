...
export class LayoutConfigService {
	...
	isMenuMinimize: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(null);
	...
	/**
	 * Save layout config to the timeNow storage
	 * @param layoutConfig
	 */
	saveConfig(layoutConfig: LayoutConfigModel): void {
		if (layoutConfig) {
			localStorage.setItem('seatellerLayoutConfig', JSON.stringify(layoutConfig));
		}
	}

	/**
	 * Get layout config from timeNow storage
	 */
	getSavedConfig(): LayoutConfigModel {
		const config = localStorage.getItem('seatellerLayoutConfig');
		try {
	 ...
	 */
	resetConfig(): void {
		localStorage.removeItem('seatellerLayoutConfig');
	}
