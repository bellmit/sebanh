...
import {
	...
	CACHE_CO_CODE, CODE_OK,
	INPUTTER_SEATELLER,
	LIST_GROUP_WITH_NAME, MILLISECOND_1_MINUTE, SLA_CONFIG_CACHE,
	STATUS_OK
} from '../../../../../../../shared/util/constant';
import {
	SeriesCircleChart, SeriesStackedBarChart,
	SlaBranchData,
	SlaReportModel,
	SlaStatusCount,
	SLaTimeConfig,
...
@Component({
	selector: 'kt-sla-history',
	templateUrl: './sla-history.component.html',
	styleUrls: ['./sla-history.component.scss', '../sla-report.component.scss']
})
export class SlaHistoryComponent implements OnInit, OnDestroy {
	...
	onSubmit() {
		const fromDate = moment(this.searchForm.get('fromDate').value).format('DD-MM-YYYY');
		const toDate = moment(this.searchForm.get('toDate').value).format('DD-MM-YYYY');
		let userStr = this.searchForm.get('user').value;
		userStr = (userStr === undefined || userStr === null) ? '' : userStr;
		...
		this.onSearchData(area, branch, fromDate, toDate, business, userStr);
	}
	...
	getSlaConfig(): void {
		...
		const groups = [this.inputterGroupName, this.authoriserGroupName];
		...
		this.apiService.getSlaTransactions(branch, fromDate, toDate, business, user, '')
			...
			.subscribe(res => {
				if (res.seabRes.body.status == STATUS_OK && res.seabRes.body.enquiry.responseCode == CODE_OK) {
					...
					// this.commonService.error('Truy vấn danh sách giao dịch không thành công ');
				}
			});
	}
	...
	filterDataByBusiness(): void {
		...
		// 			break;
		// 		case 'SUPPER':
		// 			data.listCombo = rawData.listCombo.filter(combo => ['SUPPER'].includes(combo.comboType));
		// 			data.listAccount = [];
		// 			data.listCustomer = [];
		// 			data.listSms = [];
		// 			data.listCard = [];
		...
		this.transactions$.next(rawData);
		this.calculateDataForChart();
	}

	private calculateDataForChart() {
	...
	calculateSemiCircleChat(listCombo1: SlaTranMdel[], listCombo2: SlaTranMdel[], listCombo3: SlaTranMdel[], listEbank: SlaTranMdel[],
		...
		const overSlaSupper = this.countOverSla(listSupper);
		...
		this.countOver$.next(overSlaCombo1 + overSlaCombo2 + overSlaCombo3 + overSlaSeanet + overSlaCard + overSlaSms + overSlaCustomer + overSlaAccount + overSlaSupper);
		// get half over sla
	...
	calculateSlaByBranch(listCombo1: SlaTranMdel[], listCombo2: SlaTranMdel[], listCombo3: SlaTranMdel[], listEbank: SlaTranMdel[],
		...
		const inSlas: number[] = [];
		const overSlas: number[] = [];
		const halfOverSlas: number[] = [];
		series.forEach(s => {
